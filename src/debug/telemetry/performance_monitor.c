...
int io_perf_monitor_release_slot(struct io_perf_data_item *item)
{
	...
	int ret = perf_bitmap_clearbit(&self->io_performance_data_bitmap, idx);
...
static int
io_perf_monitor_disabled(struct io_perf_monitor_ctx *self)
{
	return 0;
}
...
static int
io_perf_monitor_started(struct io_perf_monitor_ctx *self)
{
	return 0;
}

static int
io_perf_monitor_paused(struct io_perf_monitor_ctx *self)
{
	return 0;
}
...
int io_perf_monitor_set_state(enum ipc4_perf_measurements_state_set state)
{
	...
	{
		ret = io_perf_monitor_disabled(self);
		break;
	}
	case IPC4_PERF_MEASUREMENTS_STOPPED:
	{
		ret = io_perf_monitor_stopped(self);
		break;
	}
	case IPC4_PERF_MEASUREMENTS_STARTED:
	{
		ret = io_perf_monitor_started(self);
		break;
	}
	case IPC4_PERF_MEASUREMENTS_PAUSED:
	{
		ret = io_perf_monitor_paused(self);
		break;
	}
	default:
		ret = -EINVAL;
...
void io_perf_monitor_update_data(struct io_perf_data_item *slot_id, uint32_t increment)
{
	...
	if (IPC4_PERF_MEASUREMENTS_STARTED == self->state && NULL != slot_id)
